...
struct DetailSessionView: View {
    ...
    @State var confirmDeleteSession = false
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Button {
                    self.confirmDeleteSession.toggle()
                } label: {
                    Image(systemName: "trash.fill")
                }
                .alert("You sure to delete session?", isPresented: $confirmDeleteSession) {
                    Button("Cancel", role: .cancel) { }
                    Button("Yes", role: .destructive) {
                        SessionRepository.shared.remove(session)
                    }
                }
            }
        }
